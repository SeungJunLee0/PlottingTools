...
class GemGroupTable:
  ...
  def AddGemGroupVar(self, varName, varTitle=None, varTitleTex=None, varUnitTex=None):
...
#for ring in rings:
  ...
  def FillGemGroupByIndex(self, varIndex, endcap, disk, ring, data, link = None):
    self.data[varIndex][1][endcap-1][disk-1][ring-1][0] = data
    self.data[varIndex][1][endcap-1][disk-1][ring-1][1] = link
  
  def FillGemGroupByName(self, varName, endcap, disk, ring, data, link = None):
    self.FillGemGroupByIndex(self.GetVarIndex(varName), endcap, disk, ring, data, link)
  
  def FillGemGroup(self, varName, endcap, disk, ring, data, link = None):
    self.FillGemGroupByName(varName, endcap, disk, ring, data, link)
  
  def PrintData(self):
    print self.data
  
  def PrintHtml(self, htmlFileName, varNamesToPrint="Var", caption="GEM Group Table Caption", isComplete=0):
    varIndexes = []
...
#print >> htmlFile, "<th rowspan=\"9\"><i>GE%s</i></th>" % sEndcap
        print >> htmlFile, "<th><i>GE%s</i></th>" % sEndcap
      if ring == 1:
#if disk == 1: print >> htmlFile, "<th rowspan=\"3\"><i>GE%s%s</i></th>" % (sEndcap, disk)
#       else:         print >> htmlFile, "<th rowspan=\"2\"><i>GE%s%s</i></th>" % (sEndcap, disk)
        if disk == 1: print >> htmlFile, "<th><i>GE%s%s</i></th>" % (sEndcap, disk)
        else:         print >> htmlFile, "<th><i>GE%s%s</i></th>" % (sEndcap, disk)
      print >> htmlFile, "<th><i>GE%s%s/%s</i></th>" % (sEndcap, disk, ring)
      for varIndex in varIndexes:
  ...
  def PrintTex(self, texFileName, varNamesToPrint="Var", caption="GEM Group Table Caption", label="gemGroupTable", isComplete=0):
    varIndexes = []
    for varName in varNamesToPrint:
      varIndex = self.GetVarIndex(varName)
      varIndexes.append(varIndex)
...
#for ring in rings:
      if disk == 1 and ring == 1:
        print >> texFile, "\\multirow{9}{*}{\\textit{\\textbf{GE%s}}}" % sEndcap,
      if ring == 1:
        if disk == 1: print >> texFile, " & \\multirow{3}{*}{\\textit{\\textbf{GE%s%s}}}" % (sEndcap, disk),
        else:         print >> texFile, " & \\multirow{2}{*}{\\textit{\\textbf{GE%s%s}}}" % (sEndcap, disk),
      else: print >> texFile, " & ",
      print >> texFile, " & \\textit{\\textbf{GE%s%s/%s}}" % (sEndcap, disk, ring),
      for varIndex in varIndexes:
        data = self.data[varIndex][1][endcap-1][disk-1][ring-1][0]
